...
class Product
  ...
  def initialize(name, price, promotion)
    ...
    raise "Only prices between 0.01 and 999.99 allowed" unless 0 < price and
    price < 1000
...
class LineItem
  ...
  def increase(count)
    ...
    raise 'Max 99 items of each product can be bought' if count + @count > 99
    @count += count
  end
...
class InvoicePrinter
  ...
  def print_items
    @cart.items.each do |item|
      print item.product_name, item.count,"%5.2f" %(item.price_without_discount)
      print_help (item)
    end

    if @cart.coupon_discount.nonzero?
      name = "Coupon #{@cart.coupon.name} - #{@cart.coupon.description}"
      print name, '', "%5.2f" %(-@cart.coupon_discount)
    end
  end


  def print_help (item)
    print "  (#{item.discount_name})", '',
    "%5.2f" %(-item.discount) if item.discounted?
  end

  def print_total
    print_line
    print 'TOTAL', '', "%5.2f" %(@cart.total)
    print_line
  end
